...
OUT_REPORT = "/root/autodl-tmp/dnagpt/outputs/OmniGene-4-MM-stage2/eval_report.json"
...
print("=" * 60)
stage1_path = "/root/autodl-tmp/dnagpt/outputs/OmniGene-4-MM-stage1/eval_report.json"
if os.path.exists(stage1_path):
    s1 = json.load(open(stage1_path))
    for task in chebi_results:
        s1_acc = s1["vis_chebi20_test"][task]["accuracy"]
        s2_acc = chebi_results[task]["acc"]
        delta = s2_acc - s1_acc
        sign = "+" if delta >= 0 else ""
        print(f"  vis_{task:14s}: S1 {s1_acc:.3f} -> S2 {s2_acc:.3f} ({sign}{delta:+.3f})")
    print(f"  remote_homology   : S1 {s1['remote_homology_carryover']['acc']:.3f} -> S2 {rem_acc:.3f}")
print(f"  standard_homology : S2 {std_acc:.3f} (v5 baseline: 0.994)")
