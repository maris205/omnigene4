...
# coding: utf-8
"""
83-train_stage3v3.py

OmniGene-4-MM Stage 3 v3: high-LR pass with frozen embedding.

Diagnosis from v2: LR 5e-6 too low + 6000 steps with trainable embedding
yielded loss ~27 plateau and almost zero homology improvement.

v3 changes:
1. LR 2e-5 (4x v2) -- match Stage 1 effective LR, push past plateau
2. Freeze embedding (v5 baseline trained this way)
3. 3000 steps (~16h) -- 2 epochs through homology-heavy data
4. 45K text (heavy homology) + 5K vision replay (minimal)
5. Continue from Stage 2 LoRA (adapter name "stage2")
"""
import os
...
OUTPUT_DIR  = "/root/autodl-tmp/dnagpt/outputs/OmniGene-4-MM-stage3v3"
CKPT_DIR    = "/root/autodl-fs/checkpoints_omnigene4_mm_stage3v3"
...
# Sampling -- v3: heavier homology, less vision, freeze embedding
N_TEXT = 45000        # mostly homology
N_VISION = 5000       # minimal replay
HOMOLOGY_MULT = 3     # boost homology
FREEZE_EMBEDDING = True  # v5 baseline trained with frozen embedding
...
if not FREEZE_EMBEDDING:
    ...
    print(f"  Stage 2 embedding loaded + TRAINABLE ({stage2_emb.shape})")
else:
    print(f"  Stage 2 embedding loaded + FROZEN ({stage2_emb.shape})")
...
print(f"\nStage 3 v3 (high-LR + frozen embedding):")
print(f"  Init: Stage 2 LoRA properly loaded (adapter='{ADAPTER_NAME}')")
print(f"  Embedding: {'FROZEN' if FREEZE_EMBEDDING else 'trainable'}")
...
with open(META_FILE, "w") as f:
    json.dump({
        ...
        "stage": "stage3v3_high_lr_frozen_emb",
