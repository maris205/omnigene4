...
\begin{abstract}
\noindent We introduce \textbf{OmniGene-4}, a unified bio-language foundation model built on Gemma-4-26B-A4B (30 layers, 128 experts per layer, top-8 routing), and use it to address a question that has been actively debated for general LLMs but rarely measured directly in the biological domain: \emph{what does continued pretraining (CPT) do that supervised fine-tuning (SFT) cannot?} By installing forward hooks on every router and aggregating expert-routing distributions across eight task families, we provide the first router-level decomposition for a biological MoE model: under a layer-averaged Jensen--Shannon metric, CPT accounts for \textbf{96\%} of cross-task expert differentiation and SFT for \textbf{4\%}, with bootstrap 95\% CIs excluding zero for both. CPT reshapes middle transformer layers ($L_{11}$--$L_{22}$) while SFT concentrates on the final two layers ($L_{28}$--$L_{29}$), suggesting a representation versus output-alignment factorization. Within the protein-homology task family, per-pair routing JS stays below 0.04 (vs 0.23 cross-task), implying that homology decisions occur \emph{inside} expert computation rather than at the routing gate. The same training pipeline also yields strong benchmark performance: on a protein remote-homology benchmark, our model reaches \textbf{82.60\%} accuracy, outperforming ESM-2 (3B) by $+31.4$ pp, MMseqs2 by $+28.2$ pp, and DIAMOND by $+29.4$ pp on the identical pair set; standard homology and BixBench reach 99.40\% and 93.66\%. A dual-head architecture adds per-residue 3Di and DSSP classifiers reaching 78.6\% and 100\% accuracy on a held-out evaluation, without interfering with chat-style generation. Together, these results probe how mixture-of-experts foundation models acquire and organize biological multi-modal capability --- a question central to the next generation of scientific large language models.
\end{abstract}
