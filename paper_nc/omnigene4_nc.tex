...
\begin{abstract}
\noindent We introduce \textbf{OmniGene-4}, a unified bio-language foundation model on Gemma-4-26B-A4B (128 experts/layer, top-8 routing), and ask: \emph{what does continued pretraining (CPT) do that supervised fine-tuning (SFT) cannot?} By hooking every router across eight task families, we provide the first router-level decomposition for a biological MoE: CPT accounts for \textbf{96\%} of cross-task expert differentiation and SFT for \textbf{4\%}, reshaping middle and output layers respectively. Within the protein-homology task family, per-pair routing divergence stays below 0.04 (vs 0.23 cross-task), implying that homology decisions occur \emph{inside} expert computation rather than at the routing gate. The same pipeline yields strong benchmarks: on protein remote-homology our model reaches \textbf{82.60\%}, outperforming ESM-2 (3B), MMseqs2, and DIAMOND by 28--31 pp; standard homology reaches \textbf{99.40\%}; and \textbf{BixBench} --- a general biological-knowledge benchmark --- reaches \textbf{93.66\%}, showing that biological specialization does not collapse scientific-language capability. A dual-head architecture adds per-residue 3Di/DSSP classifiers reaching 78.6\%/100\%. These results probe how MoE foundation models acquire and organize biological multi-modal capability --- a question central to the next generation of scientific large language models.
\end{abstract}
