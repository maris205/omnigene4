...
BioPAWS-2 is, to our knowledge, the first biological benchmark that is simultaneously
instruction-tuning native, SFT-trainable, multimodal, and cross-modal, with a dual
zero-shot/fine-tune protocol. It subsumes the task content of TAPE, PEER, FLIP, ProteinGym,
GUE, DeepLoc, SignalP, and the LLaMA-Gene corpus into one chat-formatted axis on which a
specialist PLM (in its native head-training regime) and a generalist LLM (zero-shot or
fine-tuned) can be compared directly. By releasing it as a public training resource on
which any model can be fine-tuned and scored, BioPAWS-2 reframes the evaluation of
OmniGene-4-MM: the model is one of many entrants on an open leaderboard, not the author of
its own test --- directly addressing the self-benchmarking concern of evaluating a model on
a benchmark built by the same group.
