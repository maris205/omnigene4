...
\title{\textbf{BioPAWS-2: A Chat-Form Instruction-Tuning Benchmark and Training Resource\\ for Biological Foundation Models, with OmniGene-4 as a Case Study}}
...
\begin{abstract}
\noindent The evaluation landscape for biological foundation models is fragmented across
paradigm-specific formats --- protein/DNA language models with task-specific classification
heads (TAPE, PEER, FLIP, ProteinGym, GUE) on one side, and free-form biological question
answering on the other --- with no common axis on which a specialist encoder and a
generalist chat model can be compared. We introduce \textbf{BioPAWS-2}, a public chat-form
instruction-tuning benchmark and \emph{training resource} that re-expresses biological
sequence classification, regression, structure, variant-effect, cross-modal, reasoning, and
multimodal tasks in one instruction-QA format (9 task families, 22 tasks, 306K examples;
released on Hugging Face and GitHub). Every task ships train/val/test splits and is scored
under a dual protocol: zero-shot QA and fine-tune-then-evaluate, with the trainability gap
$\Delta$ reported. We take data integrity as a first-class requirement: an MMseqs2 leakage
audit is released with the benchmark, and all sequence tasks use entity-disjoint splits
(homology leakage reduced from 0.78 to 0.08); re-running every supervised result on the
leakage-free splits moves the headline by only $-1.4$ points, showing the leaderboard is not
a memorization artifact. As a case study we evaluate \textbf{OmniGene-4}, a bio-language
Mixture-of-Experts model on Gemma-4-26B-A4B, alongside a matched raw-Gemma-base control and
specialist baselines. A key --- and deliberately self-critical --- finding is that
biological continued-pretraining confers \emph{no material advantage under supervised
fine-tuning} on BioPAWS-2 (OmniGene 0.769 vs.\ raw-base 0.760 macro, robust across leaky and
clean splits): almost all supervised performance comes from the BioPAWS-2 data itself, which
establishes it as a model-agnostic resource. OmniGene-4's distinctive value instead lies in
its \emph{zero-shot} remote-homology detection (82.6\%, exceeding ESM-2 3B / MMseqs2 /
DIAMOND by 28--31 pp with no task training) and in a router-level decomposition of where
sequence-aware capability forms. BioPAWS-2 provides the common, leakage-controlled axis on
which such claims can be made and reproduced.
\end{abstract}
...
\input{section_biopaws2}
\input{section_leakage}
