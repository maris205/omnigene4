...
\begin{abstract}
\noindent Mixture-of-Experts (MoE) architectures offer a rare opportunity to probe the internal organization of large language models, but this affordance has not been systematically exploited in biological foundation modeling. We introduce \textbf{OmniGene-4}, a unified bio-language foundation model built on Gemma-4-26B-A4B (30 layers, 128 experts per layer, top-8 routing) by injecting 28{,}028 biological tokens (DNA and protein BPE, Foldseek 3Di, DSSP secondary structure), continuing pretraining (CPT) on a 32.5\,GB mixture of DNA, protein, natural-language and structural corpora, and a five-stage supervised fine-tuning (SFT) pipeline (v2 through v5) on 199{,}576 instruction-format examples spanning eight task families. The final model (v5) introduces a \emph{dual-head architecture} combining a generation head with two per-residue classification heads (3Di and DSSP). On standard benchmarks v5 reaches \textbf{99.40\%} accuracy on BioPAWS standard protein homology (1{,}000 pairs), \textbf{82.60\%} on remote homology (500 pairs from \texttt{protein\_pair\_remote}), and \textbf{93.66\%} on BixBench knowledge questions. Relative to its vocabulary-extended Gemma-4-Instruct baseline (85\% / 60\% / 87\%), v5 gains $+14.4$ on Standard, $+22.6$ on Remote, and $+6.7$ on BixBench. On the identical 500-pair remote-homology evaluation, v5 outperforms ESM-2 (650M) by $+32.1$ percentage points (82.6\% vs 50.5\%). The classification heads achieve \textbf{78.6\% per-residue accuracy on 3Di} (chance 5\%) and \textbf{100\% on DSSP} (chance 12.5\%), demonstrating that MoE bio-foundation models can simultaneously support chat-based generation and token-level structured prediction without interference.
...
\textbf{(2) A complete benchmark suite across eight task families.} We report accuracy on BioPAWS \citep{wang2026biopaws} standard-protein-homology classification (99.40\%), remote-homology classification (\textbf{82.60\%}), and BixBench \citep{bixbench2024} knowledge questions (93.66\%). On the same 500-pair remote-homology evaluation we ran ESM-2 (650M) \citep{lin2023evolutionary} as an external baseline; OmniGene-4 v5 outperforms it by $+32.1$ percentage points (82.6\% vs 50.5\%). The v5 dual-head architecture additionally achieves \textbf{78.6\% per-residue accuracy on 3Di} and \textbf{100\% on DSSP} (chance levels 5\% and 12.5\%), a level of structured prediction that the generation-only path cannot reach (25.9\% character overlap). The consistent BixBench performance across v3 and v5 (93.66\%) shows that our cumulative SFT pipeline does not collapse general-knowledge capability, contrasting with prior observations on dense models \citep{luo2023forgetting}.
...
All training scripts (\texttt{biopaws/cpt/1-prepare\_cpt\_data\_mp.py}, \texttt{biopaws/cpt/2-run\_cpt.py}, \texttt{biopaws/sft\_data/14-train\_bio\_sft\_v2.py}, \texttt{biopaws/sft\_data/17-train\_bio\_sft\_v3\_remote.py}), evaluation scripts (\texttt{biopaws/cpt/18-eval\_v3\_sft.py}), robustness-check scripts (\texttt{biopaws/cpt/26-format\_matched\_routing.py}, \texttt{biopaws/cpt/27-esm2\_remote\_headtohead.py}, \texttt{biopaws/cpt/28-bootstrap\_ci.py}), and analysis scripts (\texttt{biopaws/cpt/20--24*.py}) are available in the project repository. The token-level activation database and routing-count matrices are in \texttt{outputs/moe\_analysis/}. LoRA adapter weights and embedding checkpoints ($\approx 1.9$\,GB) can be released on reasonable request.
