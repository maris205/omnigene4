...
\paragraph{No format-matched control.} Our eight task pools are heterogeneous in prompt format: raw DNA and protein strings, templated homology prompts (\texttt{\#\#\# Sequence 1:} \ldots), instruction-formatted Cell/Mol/Structure prompts, and unformatted OpenWebText excerpts. Some of the observed cross-task JS divergence is therefore attributable to prompt formatting rather than biological content: Expert 100 in the token-level table activates on SMILES punctuation and instruction scaffolding alike. A clean follow-up would wrap all tasks in a single neutral template, strip distinguishing headers, and re-measure JS; only the residual signal is unambiguously content-driven.
...
\paragraph{No external benchmarking with same-split head-to-head.} Our remote-homology evaluation is performed only on a 2{,}000-pair subset of \texttt{dnagpt/biopaws::protein\_pair\_remote}. We did not re-run MMseqs2 \citep{steinegger2017mmseqs2}, DIAMOND \citep{buchfink2021diamond}, ESM-2 \citep{lin2023evolutionary}, PLMSearch \citep{liu2024plmsearch}, or CATHe \citep{nallapareddy2023cathe} on the exact same 2{,}000-pair evaluation, so all cross-method comparisons are mediated by numbers published on different splits. An apples-to-apples comparison on the same 2{,}000 pairs, plus an external evaluation on TAPE \citep{rao2019tape} or ProteinGym \citep{notin2023proteingym}, would substantially strengthen the benchmark story.
...
Based on the decomposition results and the limitations above, we propose five concrete follow-ups: \emph{(1)} a CPT-scaling study on 100\,GB of curated distant-protein pairs from SCOP \citep{murzin1995scop} or CATH fold-level annotations to test whether the representation-level bottleneck for remote homology is relieved; \emph{(2)} head-to-head same-split evaluation of MMseqs2/DIAMOND/ESM-2/PLMSearch/CATHe on the exact 2{,}000-pair remote homology subset; \emph{(3)} a format-matched routing control that wraps all tasks in a single neutral template and re-measures cross-task JS divergence to quantify prompt-format confounds; \emph{(4)} packaging the forward-hook infrastructure (\S\ref{sec:hooks}) as a reproducible command-line router-audit tool for bioinformatics practitioners; \emph{(5)} cross-architecture replication on Mixtral-8x7B \citep{jiang2024mixtral} or DeepSeek-MoE \citep{dai2024deepseekmoe} bio variants to test whether the CPT/SFT decomposition generalizes.
